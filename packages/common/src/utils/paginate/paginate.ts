...
export function getOperatorFn<T>(
  op: FilterOperator,
): (...args: any[]) => FindOperator<T> {
  switch (op) {
...
export async function paginate<T>(
  ...
  if (config.where) {
    queryBuilder = queryBuilder.andWhere(
      new Brackets((queryBuilder) => queryBuilder.andWhere(config.where)),
    ); // Postgres fix (https://github.com/ppetzold/nestjs-paginate/pull/97)
  }
